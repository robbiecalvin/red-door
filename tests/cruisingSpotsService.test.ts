...
describe("cruisingSpotsService", () => {
  ...
  it("Given a guest When creating a spot Then it rejects with ANONYMOUS_FORBIDDEN", () => {
    const svc = createCruisingSpotsService();
    const res = svc.create({ userType: "guest", ageVerified: true }, { name: "x", address: "a", lat: 1, lng: 2, description: "y" });
